...
#define n_threads 4
#define flag_size 64
#define iteration_runtime 1.0 // seconds
#define shots 10
#define warm_up 1
...
int main(int argc, char **argv)
{
    ...
    int_type lock_log[n_threads*shots +1] = {231};                 // stores order of access into CSQ[i]  by thread_ID
    int_type tid;
    int index;
    index = 0;
    ...
    #pragma omp parallel private(tid) shared(iteration_start, level, victim, competing, victim_peterson, flag_peterson, lock_log, index, max_times, min_times, avg_times, th_indices)
    {
        tid = omp_get_thread_num();
        int_type th_index=0; // in order to ma sum_val work - atomic OK?
        double max_time = 0.0;
        double avg_time = 0.0;
        double min_time = iteration_runtime;
        ...
        {     
            ...
            if (th_index >= warm_up) // ignore first rounds
            {
                if (time_dif > max_time) max_time=time_dif;
                if (time_dif < min_time) min_time=time_dif;
            }
        ...
        #pragma omp single
        {
            ...
            */
            ...
            printf("tid acquisitions min_time max_time avg_time\n");
            for (int i=0; i<n_threads; i++) printf("%d %d %f %f %f\n", i, th_indices[i], min_times[i], max_times[i], avg_times[i]);
            ...
            */
            ...
            printf("\nlock_log:\n");
            for (int i=0; i<n_threads*shots; i++) printf("%d ", lock_log[i]);
            printf("\n");
        }
    }
